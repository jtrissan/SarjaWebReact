...
def hae_pelaajan_tiedot(pelaaja_id):
    return Pelaaja.query.filter_by(id=pelaaja_id).first()

def hae_pelaajan_ottelut(pelaaja_id):
    ottelutiedot = []
    ottelut = Ottelu.query.filter((Ottelu.pelaaja1_id == pelaaja_id) | (Ottelu.pelaaja2_id == pelaaja_id)).all()
